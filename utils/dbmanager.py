...
class DatabaseManager:
    ...
    def delete_old_execution_logs(self, cutoff_date):
        """
        Delete all rows from execution_logs older than the given cutoff_date.
        cutoff_date can be a string in 'YYYY-MM-DD HH:MM:SS' format or a datetime object.
        ...
        if isinstance(cutoff_date, str):
            try:
                ...
                self.logger.error("cutoff_date must be a datetime object or a string in 'YYYY-MM-DD HH:MM:SS' format.")
                return 0
        stmt = "DELETE FROM execution_logs WHERE datetime < %s"
        self.cur.execute(stmt, (cutoff_date.strftime("%Y-%m-%d %H:%M:%S"),))
...
def parse_args():
    ...
    delete_parser = sub.add_parser(
        ...
        help="Delete all execution_logs rows older than the specified date."
    )
    delete_parser.add_argument(
        "--before",
        required=True,
        help="Delete logs older than this date (format: 'YYYY-MM-DD HH:MM:SS')."
    )
